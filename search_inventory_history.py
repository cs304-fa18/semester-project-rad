...
def getAllInventoryHistoryInfo(conn, rowType='dictionary'):
    ...
    curs.execute(
        '''select item_id, status, relevance, `type` from inventory''')
...
def getInventoryByStatus(conn, status, rowType='dictionary'):
    ...
    if rowType == "tuple":
        ...
        # results as Dictionaries
        curs = conn.cursor(MySQLdb.cursors.DictCursor)
    curs.execute('''select item_id, status, relevance, `type` from inventory
    where status = %s''', (status))
    return curs.fetchall()
    
# def getInventoryByType(conn, itemType, rowType='dictionary'):
